...
- **2026-06-08** — intento de fix de normalización de RBD en RLS de escritura: la migración `20260608_consejos_storage_evidencias_write_normalized_rbd.sql` creó `has_school_write_access_storage_key()` y la asignó a las políticas de `INSERT/UPDATE/DELETE`. La función manejaba `replace(rbd, '/', '-')` correctamente en SQL, pero falla sistemáticamente en el contexto real del servicio de storage de Supabase con `new row violates row-level security policy` aunque el usuario tenga rol, correo y RBD totalmente correctos en `usuario_establecimiento_roles`. Diagnóstico confirmó: función en BD correcta, usuario `activo = true`, `rbd_normalizado` coincidente, email idéntico en `auth.users`. La causa exacta es incompatibilidad de la función con el contexto de ejecución interna del storage service de Supabase. **Ver fix definitivo 2026-06-19 a continuación.**
- **2026-06-19** — fix definitivo para subida de evidencias en escuelas con RBD tipo anexo (`/`): la migración `20260619_consejos_storage_write_allow_representante_domain.sql` revierte las políticas de `INSERT/UPDATE/DELETE` de `evidencias_actas` de vuelta a `has_school_scope_access_storage_key()` (creada en `20260526_consejos_storage_scope_normalized_rbd.sql`), que ya manejaba `replace(scoped.rbd, '/', '-') = target_storage_key` y funciona correctamente en todos los contextos incluyendo el storage service. Se incluye también la política de `SELECT` (`Lectura evidencias por rbd`) que había quedado en `has_school_scope_access()` sin `nullif` ni normalización, causando el 400 en GET para los mismos RBDs con `/`. **Invariante resultante: las cuatro políticas de `evidencias_actas` (SELECT/INSERT/UPDATE/DELETE) deben usar la misma función `has_school_scope_access_storage_key(nullif(split_part(name, '/', 1), ''))`. Nunca crear helpers de escritura separados para `storage.objects` con condiciones adicionales.**
...
- ~~**2026-06-08**~~ confirmación de `20260608` superada: esta migración introduce una regresión — ver hallazgo 2026-06-19
- confirmación en entorno real de que `20260619_consejos_storage_write_allow_representante_domain.sql` está aplicada; sin ella, todos los uploads de evidencias a escuelas con RBD tipo anexo (`/`) fallan con `new row violates row-level security policy` en INSERT aunque el usuario tenga acceso correcto
...
| `supabase/migrations/20260608_consejos_storage_evidencias_write_normalized_rbd.sql` | **SUPERADA por `20260619`** — introduce `has_school_write_access_storage_key()` para escritura en storage; falla en el contexto del storage service de Supabase aunque los datos sean correctos; no aplicar en entornos nuevos |
| `supabase/migrations/20260619_consejos_storage_write_allow_representante_domain.sql` | **fix definitivo de storage** — revierte las cuatro políticas de `evidencias_actas` (SELECT/INSERT/UPDATE/DELETE) a `has_school_scope_access_storage_key(nullif(split_part(name, '/', 1), ''))`, que ya maneja RBDs con `/` y funciona en todos los contextos; elimina `has_school_write_access_storage_key` de las políticas de storage |
...
- si no está aplicada `20260619_consejos_storage_write_allow_representante_domain.sql`, la **lectura y escritura** (subir/actualizar/eliminar PDF y GET de archivo) fallan con `new row violates row-level security policy` o `400` para escuelas con RBD tipo anexo (`/`); la migración `20260608` que intentó corregir esto introduce una regresión y no debe usarse como estado final — la `20260619` es la que consolida las cuatro políticas en `has_school_scope_access_storage_key`
- **lección de diagnóstico**: si un upload de storage falla con `new row violates row-level security policy` y todos los datos en BD son correctos (función en BD correcta, usuario activo, RBD normalizado coincidente, email en `auth.users` igual al de `usuario_establecimiento_roles`), el problema es la función de política en sí, no los datos. El servicio de storage de Supabase tiene un contexto de ejecución distinto al de PostgREST y puede fallar funciones que pasan en el SQL Editor. La solución es usar `has_school_scope_access_storage_key()` que tiene historial de funcionar correctamente en ese contexto
